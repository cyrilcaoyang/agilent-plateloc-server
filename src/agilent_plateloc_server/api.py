...
Agent documentation (always available, no claim required):

* ``GET /agent-docs``               - agent guide (``text/markdown``)
* ``GET /agent-docs/api-reference`` - API reference (``text/markdown``)
...
def create_app(
    *,
    ...
    # ---- Agent documentation ---------------------------------------------
    # Markdown guide + API reference + /llms.txt, served from package data.
    # Read-only and unclaimed: an agent must be able to learn how to drive the
    # device before it holds (or can hold) a claim.
